...
class mytimer {
private:
    std::chrono::steady_clock::time_point _begin; // 开始时间
    std::chrono::steady_clock::time_point _end;   // 结束时间
public:
    mytimer() {
...
static uint32_t Table2[256] = {
    ...
    0xB4B4992D, 0xD1D1A475, 0x434390D3, 0x48485A12, 0xE2E2E258, 0x979771E6,
...
static void SM4_inner(uint8_t* in, uint8_t* out, SM4_Key sm4_key, uint8_t enc) {
    ...
    for (int i = 0; i < 4; i++) {
        int j = 4 * i;
        x[i] = (in[j + 0] << 24) | (in[j + 1] << 16) | (in[j + 2] << 8) | (in[j + 3]);
    }
    // 32轮循环
    for (int i = 0; i < 32; i++) {
...
void SM4_Encrypt(uint8_t* plaintext, uint8_t* ciphertext, SM4_Key sm4_key) {
    SM4_inner(plaintext, ciphertext, sm4_key, 0);
}

// SM4解密
void SM4_Decrypt(uint8_t* ciphertext, uint8_t* plaintext, SM4_Key sm4_key) {
    SM4_inner(ciphertext, plaintext, sm4_key, 1);
}

// 删除密钥
void SM4_Delete(SM4_Key sm4_key) {
    free(sm4_key);
}
